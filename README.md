...
## Instructions:
- Input_prompts will be checked if there is Chinese_text generate to decide whether auto load translator or not. If input_prompts is English but want to generate Chinese_text, then the Chinese_text must in the（Full-width characters）`(“”)` , not（Half-width characters）`("")`.
